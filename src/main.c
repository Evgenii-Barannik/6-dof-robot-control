...
typedef struct { // Array of pointers to NULL (if slider position is not possible) or slider positions (if slider position is possible).
...
void print_points(gsl_vector **points, int len) {
    for (int i = 0; i < len; i++) {
        printf("%d: (%f, %f, %f)", i, 
